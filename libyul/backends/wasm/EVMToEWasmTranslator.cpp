...
function u265_to_i64(x1, x2, x3, x4) -> v {
	if i64.ne(0, i64.or(i64.or(x1, x2), x3)) { invalid() }
	v := x4
}

function u265_to_i32(x1, x2, x3, x4) -> v {
	if i64.ne(0, i64.or(i64.or(x1, x2), x3)) { invalid() }
	// TODO also check the higher oder bits of x4
	v := x4
}

function u265_to_i32ptr(x1, x2, x3, x4) -> v {
	if i64.ne(0, i64.or(i64.or(x1, x2), x3)) { invalid() }
	// TODO also check the higher oder bits of x4
	v := x4
}
...
function calldataload(x1, x2, x3, x4) -> z1, z2, z3, z4 {
	let t1, t2, t3, t4 := save_temp_mem_32()
	eth.callDataCopy(0, u265_to_i32(x1, x2, x3, x4), 32)
	z1, z2, z3, z4 := mload(0, 0, 0, 0)
	restore_temp_mem_32(t1, t2, t3, t4)
}
...
function calldatacopy(x1, x2, x3, x4, y1, y2, y3, y4, z1, z2, z3, z4) {
	eth.callDataCopy(
		u265_to_i32ptr(x1, x2, x3, x4),
		u265_to_i32(y1, y2, y3, y4),
		u265_to_i32(z1, z2, z3, z4)
	)
}
...
function selfdestruct(a1, a2, a3, a4) {
	...
	// In EVM, addresses are padded to 32 bytes, so discard the first 8.
	eth.selfDestruct(8)
}
